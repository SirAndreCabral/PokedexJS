...
const buscarEvolucoes = async (urlEspecie) => {
  try {
    ...
    const dadoEvolucao = await respostaEvolucao.json();

    let cadeia = dadoEvolucao.chain;
    const evolucoes = [];

    do {
      const respostaPokemon = await fetch(
        `https://pokeapi.co/api/v2/pokemon/${cadeia.species.name}`
      );
...
const exibirEvolucoes = async (evolucoes) => {
  listaEvolucoes.innerHTML = "";

  if (evolucoes.length <= 1) {
    listaEvolucoes.innerText = "Este Pokemon não possui evoluções.";
    return;
  }
  ...
  evolucoes.forEach((evolucao) => {
    ...
    fotoEvolucao.alt = `Foto do pokemon ${evolucao.nome}`;
...
const buscarPokemon = async (query) => {
  ...
  try {
    ...
    tipoPokemon.innerText = pokemon.types
